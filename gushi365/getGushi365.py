	...
	for page in range(int(start_page), int(end_page)+1):
	    ...
	    for link in links:
	        ...
	        while retries < max_retries:
	            try:
	                ...
	                if response.status_code == 200:
	                    # 处理抓取到的数据
	                    print(f'Successfully crawled data from {url}')
	                    break  # 如果成功抓取到数据，跳出循环
	                else:
	                    # 如果返回的状态码不是200，抛出异常并进行重试
	                    raise requests.exceptions.RequestException(f'Invalid status code {response.status_code}')
	            except requests.exceptions.RequestException as e:
	                retries += 1
	                print(f'Error crawlingURL {url}: {e}')
	                if retries < max_retries:
	                    # 如果未达到最大重试次数，等待一段时间后进行重试
	                    print(f'Retrying {retries} time(s) after {time.sleep(3)} seconds')
	                else:
	                    # 如果已达到最大重试次数，跳过当前URL，处理下一个URL
	                    print(f'Reached maximum retries for {url}, skipping...')
	                    skip = 1
	                    #生成抓取失败的记录
	                    formatted_log = f'RetryFail,,,,,来源URL：{detail_url}\n'
	                    break
	        ...
	        if title_tag:
	            ...
	            link_div = soup.find('nav', class_='breadcrumb')
	            ...
	            #生成抓取成功的记录
	            formatted_log = f'Success,故事名称：{story_name},标准分类：{standard_category},原始分类1：{raw_category1},原始分类2：{raw_category2},来源URL：{detail_url}\n'
